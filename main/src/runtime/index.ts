...
import { V8CoverageReporter } from "../adapters/V8CoverageReporter.js"
...
export async function run(args: RunArguments): Promise<RunResult> {
  ...
  const coverageReporter = args.coverageReporter ? new V8CoverageReporter(args.coverageReporter) : new NullCoverageReporter()
  ...
  const coverageManager = args.collectCoverage ? new CoverageManager(viteTranspiler, coverageReporter, behaviorBrowser) : undefined
